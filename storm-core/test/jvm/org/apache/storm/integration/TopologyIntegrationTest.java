...
public class TopologyIntegrationTest {
    ...
    public void testBasicTopology(boolean useLocalMessaging) throws Exception {
        try (LocalCluster cluster = new LocalCluster.Builder()
            ...
            assertThat(Testing.readTuples(results, "1"), containsInAnyOrder(testValues.toArray()));
    ...
    public void testSystemStream() throws Exception {
        ...
        try (LocalCluster cluster = new LocalCluster.Builder()
            ...
            List<Values> testValues = new ArrayList<>();
            testValues.add(new Values("a"));
            testValues.add(new Values("b"));
            testValues.add(new Values("c"));
            List<FixedTuple> testTuples = testValues.stream()
                .map(value -> new FixedTuple(value))
                .collect(Collectors.toList());
    ...
    public void testKryoDecoratorsConfig() throws Exception {
        ...
        try (LocalCluster cluster = new LocalCluster.Builder()
            ...
            List<Values> testValues = new ArrayList<>();
            testValues.add(new Values(Config.TOPOLOGY_KRYO_DECORATORS));
            List<FixedTuple> testTuples = testValues.stream()
                .map(value -> new FixedTuple(value))
                .collect(Collectors.toList());
    ...
    public void testComponentSpecificConfig() throws Exception {
        ...
        try (LocalCluster cluster = new LocalCluster.Builder()
            ...
            List<Values> testValues = new ArrayList<>();
            testValues.add(new Values("fake.config"));
            testValues.add(new Values(Config.TOPOLOGY_MAX_TASK_PARALLELISM));
            testValues.add(new Values(Config.TOPOLOGY_MAX_SPOUT_PENDING));
            testValues.add(new Values("fake.config2"));
            testValues.add(new Values(Config.TOPOLOGY_KRYO_REGISTER));
            List<FixedTuple> testTuples = testValues.stream()
                .map(value -> new FixedTuple(value))
                .collect(Collectors.toList());
